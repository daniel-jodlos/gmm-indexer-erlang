-module(indexed_queries_simple_SUITE).
...
operations_test(_Config) ->
    ...
    {ok, #{<<"members">> := MembersResponse2}} = client:indexed_members(<<"zone1:Group2">>),
    {ok, #{<<"members">> := MembersResponse3}} = client:indexed_members(<<"zone0:Space1">>),
    {ok, #{<<"members">> := MembersResponse4}} = client:indexed_members(<<"zone0:Provider1">>),

    {ok, #{<<"reaches">> := ReachesResponse1}} = client:indexed_reaches(<<"zone1:User3">>, <<"zone2:Provider3">>),
    {ok, #{<<"reaches">> := ReachesResponse2}} = client:indexed_reaches(<<"zone2:User5">>, <<"zone2:Space4">>),
    {ok, #{<<"reaches">> := ReachesResponse3}} = client:indexed_reaches(<<"zone1:User3">>, <<"zone1:Provider2">>),
    {ok, #{<<"reaches">> := ReachesResponse4}} = client:indexed_reaches(<<"zone0:User1">>, <<"zone0:Provider1">>),

    {ok, #{<<"effectivePermissions">> := EPResponse1}} =
        client:indexed_effective_permissions(<<"zone0:User1">>, <<"zone0:Provider1">>),
    {ok, #{<<"effectivePermissions">> := EPResponse2}} =
        client:indexed_effective_permissions(<<"zone2:User5">>, <<"zone1:Provider2">>),
    ...
    ?assertEqual([<<"zone0:User2">>, <<"zone1:User3">>, <<"zone1:Group3">>, <<"zone2:User4">>, <<"zone2:User5">>, <<"zone2:Group4">>], MembersResponse1),
